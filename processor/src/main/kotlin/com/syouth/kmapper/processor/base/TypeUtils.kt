...
internal fun areSameSupportedCollectionTypes(first: KSType, second: KSType): Boolean {
    if (first.declaration.qualifiedName?.asString() != second.declaration.qualifiedName?.asString()) {
        return false
    }
    if (first.declaration.qualifiedName?.asString() !in SUPPORTED_CONVERSION_INTERFACES) {
        return false
    }
    ...
    if (!first.arguments.zip(
            ...
            checkSameTypeWithNullabilitySufficient(
                f.type?.resolve(),
                s.type?.resolve()
            )
        }
...
internal fun areSameSupportedMapCollectionTypes(first: KSType, second: KSType): Boolean {
    ...
    return checkMapCollectionTypeKeyArgumentsNullabilitySufficient(first, second) &&
        checkSameTypeWithNullabilitySufficient(
            first.arguments.firstOrNull()?.type?.resolve(),
            second.arguments.firstOrNull()?.type?.resolve()
        )
}

internal fun KSType.getCorrespondingConcreteTypeForSupportedCollectionType(): TypeName {
    val qualifiedName =
        declaration.qualifiedName?.asString() ?: error("Can't resolve qualified name")
    if (qualifiedName in SUPPORTED_CONVERSION_INTERFACES || qualifiedName in SUPPORTED_MAP_CONVERSION_INTERFACES) {
        val concreteType =
            SUPPORTED_CONVERSION_INTERFACES_CORRESPONDING_CONCRETE_TYPES[qualifiedName]
                ?: error("Can't find concrete type for collection type")
        val indexOfLastDot = concreteType.lastIndexOf(".")
        if (indexOfLastDot == -1) error("Wrong type qualified name")
        ...
        return if (qualifiedName in SUPPORTED_CONVERSION_INTERFACES) {
            ...
            ClassName(packageName, typeName).parameterizedBy(
                arguments[0].toTypeName(),
                arguments[1].toTypeName()
            )
        }
    } else {
        error("Collection type not supported")
    }
}

internal fun checkCollectionArgumentsNullabilitySufficient(from: KSType?, to: KSType?): Boolean {
    if (from == null || to == null) error("Types can not be null")
    val fromArgument = from.arguments.firstOrNull()?.type?.resolve()
        ?: error("Collection should have arguments ${from.declaration.simpleName.asString()}")
    val toArgument = to.arguments.firstOrNull()?.type?.resolve()
        ?: error("Collection should have arguments ${to.declaration.simpleName.asString()}")
    return checkDifferentTypesNullabilitySufficient(fromArgument, toArgument)
}

internal fun checkMapCollectionTypeArgumentsNullabilitySufficient(
    from: KSType?,
    to: KSType?
): Boolean {
    if (from == null || to == null) error("Types can not be null")
    val fromArgument = from.arguments.getOrNull(1)?.type?.resolve()
        ?: error("Collection should have arguments ${from.declaration.simpleName.asString()}")
    val toArgument = to.arguments.getOrNull(1)?.type?.resolve()
        ?: error("Collection should have arguments ${to.declaration.simpleName.asString()}")
    return checkDifferentTypesNullabilitySufficient(fromArgument, toArgument)
}

internal fun checkMapCollectionTypeKeyArgumentsNullabilitySufficient(
    from: KSType?,
    to: KSType?
): Boolean {
    if (from == null || to == null) error("Types can not be null")
    val fromKeyArgument = from.arguments.firstOrNull()?.type?.resolve()
        ?: error("Collection should have arguments ${from.declaration.simpleName.asString()}")
    val toKeyArgument = to.arguments.firstOrNull()?.type?.resolve()
        ?: error("Collection should have arguments ${to.declaration.simpleName.asString()}")
...
internal fun checkDifferentTypesNullabilitySufficient(from: KSType?, to: KSType?): Boolean {
    if (from == null || to == null) error("Types can not be null")
    return from.nullability == Nullability.NOT_NULL ||
        (from.nullability == Nullability.NULLABLE && to.nullability == Nullability.NULLABLE)
}
...
internal fun KSType.extractSupportedCollectionTypeArgumentType(): KSType =
    arguments.firstOrNull()?.type?.resolve()
        ?: error("Can't find collection type argument for ${this.toClassName().simpleName}")

internal fun KSType.extractSupportedMapCollectionTypeArgument(): KSType =
    arguments
        .getOrNull(1)?.type?.resolve()
        ?: error("Can't find map collection type argument for ${this.toClassName().simpleName}")
